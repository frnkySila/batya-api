...
Многие из запросов протокола возвращают помимо прочего сообщения, отправленные кому-то там или там [REDACTED] пойми короче. Во всех случаях JSON-объект сообщения имеет один и тот же формат:
...
2.20. Драньковская проверка наличия новых сообщений (long polling)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Более простой в реализации long polling-запрос, добавленный для прямой совместимости с Павлом Драньковым. Запрос возвращает пустой ответ в случае наличия новых сообщений.

* **Method/URL:** :code:`GET /:auth_token/check_updates`
